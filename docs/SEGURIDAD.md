...
- **Documentos de RH (expedientes, formatos, solicitudes)**: mismo criterio — `disk`/`path` (o `foto_path`/`cv_path`/`firma_path`) están en `$hidden` en todos los modelos que referencian el disco `nas` (`User`, `EmployeeDocument`, `GeneratedDocument`, `DocumentTemplate`, `Candidato`, `AltaDigital`, `AltaDigitalDocumento`, `SolicitudInternaDocumento`), así que nunca viajan al frontend aunque el modelo se serialice completo (por ejemplo, `auth.user` compartido en cada request vía `HandleInertiaRequests`, o una relación cargada con `->load()` y pasada directamente a una vista sin mapear campo por campo). Las fotos/documentos se sirven siempre por una ruta protegida por policy que resuelve la ruta física en el servidor (`Rh\ExpedienteController::descargarFoto`, `Rh\EmployeeDocumentController::descargar`, etc.), nunca por URL directa al disco.
